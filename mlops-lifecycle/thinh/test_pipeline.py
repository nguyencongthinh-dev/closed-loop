...
def test_detect_drift_no_drift(normal_df):
    ...
    result = detect_drift(
        normal_df, cur_df, threshold=0.15, report_label="test-no-drift"
    )
...
def test_detect_drift_with_drift(normal_df, drifted_df):
    ...
    result = detect_drift(
        normal_df, drifted_df, threshold=0.15, report_label="test-drift"
    )
    assert isinstance(result, DriftResult)
...
def test_drift_result_has_feature_details(normal_df, drifted_df):
    ...
    result = detect_drift(
        normal_df, drifted_df, threshold=0.15, report_label="test-details"
    )
...
def test_train_model_predictions_shape(normal_df):
    ...
    model, scaler, _, _ = train_model_on_df(
        normal_df, contamination=0.03, n_estimators=10
    )
...
class TestPredictRequestValidation:
    ...
    def test_valid_structured_request(self):
        ...
        req = PredictRequest(
            features=[{"latency_p99": 120.0, "error_rate": 0.01, "rps": 450.0}]
        )
    ...
    def test_accepts_boundary_error_rate(self):
        ...
        _, FeatureRow = self._get_schemas()
        assert (
            FeatureRow(latency_p99=120.0, error_rate=0.0, rps=450.0).error_rate == 0.0
        )
        assert (
            FeatureRow(latency_p99=120.0, error_rate=1.0, rps=450.0).error_rate == 1.0
        )
...
class TestLOFDetector:
    ...
    def test_lof_returns_valid_outputs(self, normal_df):
        ...
        lof_model, anomaly_rate, mean_lof = train_lof(
            X_scaled, n_neighbors=20, contamination=0.03
        )
